...
async function fundAddress(dapiClient, faucetAddress, faucetPrivateKey, address) {
  ...
  const transactionId = await dapiClient.sendTransaction(
...
async function createIdentity(dapiClient, privateKey, type) {
  ...
  const { items: inputs } = await dapiClient.getUTXO(lockAddress.toString());
  ...
  await dapiClient.sendTransaction(
    Buffer.from(lockTransaction.serialize(), 'hex'),
  );
