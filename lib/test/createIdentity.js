 ...
 */
async function createIdentity(dapiClient, type) {
  // Create new keys
  ...
  const address = Address
    .fromPublicKey(publicKey, process.env.NETWORK === 'devnet' ? 'testnet' : process.env.NETWORK);
  ...
  await fundAddress(dapiClient, faucetAddress, faucetPrivateKey, address);

  // Create Lock Transaction
  const { items: inputs } = await dapiClient.getUTXO(address.toString());

  const lockTransaction = new Transaction();

  lockTransaction.from(inputs)
